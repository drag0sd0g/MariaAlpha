...
class FokOrderHandlerTest {

  private FokOrderHandler handler;

  @BeforeEach
  void setUp() {
    handler = new FokOrderHandler();
  }

  @Test
  void supportedTypeIsFok() {
    assertThat(handler.supportedType()).isEqualTo(OrderType.FOK);
  }
  ...
  void validateFailsBuyBelowAsk() {
    ...
    var result = handler.validate(order, marketState("149.50", "150.50"));
    assertThat(result.valid()).isFalse();
    assertThat(result.reason()).contains("not marketable");
  }
  ...
  @Test
  void validateSucceedsMarketableBuy() {
  ...
  @Test
  void toExecutionInstructionEmitsFokTif() {
    var order = createOrder(Side.BUY, 100, new BigDecimal("151.00"));
    var instr = handler.toExecutionInstruction(order);
    assertThat(instr.timeInForce()).isEqualTo(TimeInForce.FOK);
  ...
  private Order createOrder(Side side, int qty, BigDecimal limitPrice) {
    return new Order(
        new OrderSignal(
            "AAPL", side, qty, OrderType.FOK, limitPrice, null, "MANUAL", Instant.now()));
  }
