...
/**
 * Per-symbol reference data used by the Phase-2 risk checks (issues 2.2.1, 2.2.2, 2.2.3): sector
 * classification, beta vs. a benchmark, and Average Daily Volume in shares.
 *
 * <p>The MVP simulator carries a tiny fixed universe (AAPL, MSFT, GOOGL, AMZN, TSLA, NVDA), so the
 * data is statically configured under {@code execution-engine.risk.reference-data.*} rather than
 * being fetched from a vendor (Bloomberg, Refinitiv) at runtime. Production deployments would back
 * this with a periodic refresh from a corporate-actions / index-provider feed.
 *
