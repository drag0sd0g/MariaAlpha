...
@Component
public class GtcOrderHandler implements OrderTypeHandler {
  ...
  public ValidationResult validate(Order order, MarketState marketState) {
    ...
    if (order.getLimitPrice() == null || order.getLimitPrice().compareTo(BigDecimal.ZERO) <= 0) {
      return ValidationResult.fail("GTC order requires a positive limit price");
    }
    if (marketState == null) {
  ...
  public ExecutionInstruction toExecutionInstruction(Order order) {
    return new ExecutionInstruction(order, TimeInForce.GTC, order.getLimitPrice());
  }
}
