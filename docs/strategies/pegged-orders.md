...
- **Residual resubmission is NOT threshold-gated.** When the active child completes (fills fully or is cancelled by the venue) while the parent still has remaining quantity, the next NBBO tick submits a fresh child for the residual regardless of how little the price moved — there is no working child to preserve, so waiting for a full threshold move would stall the parent in a quiet market. Re-pegged and resubmitted children are always sized from the registry's `PeggedProgress.remainingQuantity()` (fills accrue on the children, never on the parent `Order` object).
